...
let socket = null;
let selectedAlgorithm = elements.algorithmSelector.value;
let isSeeking = false;
...
const startWebSocket = () => {
  ...
  socket.onmessage = (event) => {
    ...
    if (data.frame) {
      ...
      console.error("Alınan frame verisi tanımlı değil:", data);
    }
  };
  ...
  socket.onclose = () => {
    ...
    if (reconnectAttempts < maxReconnectAttempts) {
      ...
      console.log(`Bağlantı ${delay / 1000} saniye sonra tekrar deneniyor...`);
      reconnectAttempts++;
...
const togglePlayPause = () => {
  if (elements.playPauseButton.innerHTML.includes("M8,5.14V19.14L19,12.14L8,5.14Z")) {
    elements.playPauseButton.innerHTML = `
      <svg viewBox="0 0 24 24">
...
elements.algorithmSelector.addEventListener("change", () => {
  selectedAlgorithm = elements.algorithmSelector.value;
  sendSettings();
});
